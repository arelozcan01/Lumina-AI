...
STRICT_FORBIDDEN_PATTERNS = [
    # Cinsel içerik / NSFW
    r"(nsfw|porno|pornograf|nude|çıplak|adult|hentai|erotik|seks\b|sex\b|sik\b|sikiş|amcık|yarrak|orospu|piç\b|pezevenk|fahişe)",
    # Şiddet / Katliam / Tehdit / Cinayet / İntihar
    r"(katliam|katlet|öldür|cinayet|vahşet|işkence|boğazla|doğra|canına kıy|intihar|geber|şiddet|kafasına sık|kan banyosu)",
    # Terör / Patlayıcı / Ağır Zarar
    r"(bomba yap|patlayıcı|zehir üret|suikast|terör|saldırı planı|tecavüz|taciz)",
    # Ağır Küfür / Hakaret
...
def check_content_safety(text: str):
    ...
    for pattern in STRICT_FORBIDDEN_PATTERNS:
        if re.search(pattern, lowered, re.IGNORECASE):
            logger.info("Katı güvenlik filtresi tetiklendi.")
...
def enhance_image_prompt(raw_prompt: str, style: str = "photorealistic", custom_api_key: str | None = None) -> str:
    style_guidelines = {
        "photorealistic": "8k uhd, photorealistic photography, hyper-detailed skin texture, realistic natural lighting, shot on 35mm Hasselblad f/1.8 lens, sharp focus, award-winning national geographic photo, high dynamic range",
        "cinematic": "cinematic movie still, anamorphic lens, 70mm film grain, dramatic moody volumetric lighting, atmospheric fog, color graded, masterpiece",
        "anime": "high quality modern anime aesthetic, Makoto Shinkai style, vibrant luminous colors, detailed background, anime art station trending",
        "3d_render": "3D digital character render, Octane render, Unreal Engine 5, ray tracing reflections, cinematic studio light, smooth clay and subsurface scattering",
        "digital_art": "digital painting concept art, detailed oil and acrylic brushstrokes, fantasy epic illustration, vibrant colors"
    }
    ...
    enhancer_messages = [
        {
            "role": "system",
            "content": "You are an expert AI prompt engineer for Flux models. Translate and expand user's idea into an ultra-detailed English image prompt. Output ONLY the prompt."
        },
...
async def chat_endpoint(
    ...
    # 1. Tavizsiz İçerik Güvenliği Kontrolü
